...
function PickBackground(mainPage) {
	mainURL = '/mods/trigger-cookies/assets/img/';
	...
	if (width > 750) {
		if (rand2 < step2 * 2) {
			shower = 1;
			document.getElementById('bgOverlay').style.backgroundImage = 'url("' + mainURL + 'cookieShower1.png")';
		}
		else if (rand2 < step2 * 2) {
			shower = 2;
			document.getElementById('bgOverlay').style.backgroundImage = 'url("' + mainURL + 'cookieShower2.png")';
		}
		else if (rand2 < step2 * 2.1) {
			shower = 3;
			document.getElementById('bgOverlay').style.backgroundImage = 'url("' + mainURL + 'cookieShower3.png")';
		}
	}
	...
	if (rand < step * 1) {
		theme = 'store';
		document.body.style.backgroundImage = 'url("' + mainURL + 'panelBG.png")';
		if (mainPage) {
		...
		theme = 'ascend';
		document.body.style.backgroundImage = 'url("' + mainURL + 'starbg.jpg")';
		if (mainPage) {
		...
		theme = 'evil';
		document.body.style.backgroundImage = 'url("' + mainURL + 'grandmas2.jpg")';
		var rand4 = Math.random();
		var step4 = 0.035;
		if (rand4 < step4 * 1)
			document.body.style.backgroundImage = 'url("' + mainURL + 'grandmas1.jpg")';
		else if (rand4 < step4 * 2)
			document.body.style.backgroundImage = 'url("' + mainURL + 'grandmas3.jpg")';
		if (mainPage) {
		...
		theme = 'money';
		document.body.style.backgroundImage = 'url("' + mainURL + 'bgMoney.jpg")';
		if (mainPage) {
...
function PickMilk() {
	...
	document.getElementById('milk').style.backgroundImage = 'url("' + mainURL + milk + '.png")';
}
...
function AddCookie() {
	...
	for (var i = 1; i <= 6; i++) {
		if (width > 750 && ((Math.random() < 0.75 / (i * i) && i <= 3) || (Math.random() < 0.9 && i > 3 && theme == 'evil'))) {
			...
			cookie.style.backgroundImage = 'url("' + mainURL + image + '")';
